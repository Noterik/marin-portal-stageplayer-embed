...
const readStage = file => fs.readFile(file, 'utf-8')
  .then(JSON.parse);
...
const fixPath = (stagePath) => {
  ...
  return R.evolve({
    path: R.pipe(
      R.tap(p => LOGGER.debug(`Fixing path "${p}", stage dir = "${stageDir}".`)),
      ...
      p => (p.match(/.+.(sbf|h5m)$/gmi) ? path.resolve(stageDir, p) : p),
...
const getStage = (p) => {
  ...
  return Promise.all([
    ...
    return R.pipe(
      ...
      handler,
      R.prop('stage'),
      R.over(R.lensPath(['files', 'entities', 'files', 'byId']), R.map(fixPath(resolvedP))),
